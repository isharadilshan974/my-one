...
if supabase is not None:
    ...
    # Streamlit reruns recreate the Supabase client, so restore the
    # authenticated session on every rerun before using RLS-protected tables.
    ...
    # If no session is stored yet, try the client's current session.
    ...
    # Final verification of the authenticated user.
    if st.session_state.auth_user is None:
        try:
...
/* Keep Streamlit controls visible so the sidebar can always be reopened. */
